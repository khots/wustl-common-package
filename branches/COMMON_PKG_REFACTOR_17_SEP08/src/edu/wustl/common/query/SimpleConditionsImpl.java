	...
	public String getString(int tableSufix) throws SQLException
	{
		...
		for (int i = 0; i < whereConditions.size(); i++)
		{
			...
			if (i != whereConditions.size() - 1
					&& !((SimpleConditionsNode) whereConditions.get(i + 1)).getCondition()
							.getDataElement().getField().equals(Constants.ACTIVITY_STATUS_COLUMN))
				whereConditionsString.append((simpleConditionsNode).toSQLString(tableSufix));
			else if (hasConditionsExceptActivityStatus())
				whereConditionsString.append((simpleConditionsNode).getCondition().toSQLString(
						tableSufix)
						+ " ) ");
			whereConditionsString.append(" ");
		}
		...
		for (int i = 0; i < activityStatusConditions.size(); i++)
		{
			if (i != activityStatusConditions.size() - 1)
				whereConditionsString.append(((SimpleConditionsNode) activityStatusConditions
						.get(i)).toSQLString(tableSufix));
			else
				whereConditionsString.append(((SimpleConditionsNode) activityStatusConditions
						.get(i)).getCondition().toSQLString(tableSufix));
		}
