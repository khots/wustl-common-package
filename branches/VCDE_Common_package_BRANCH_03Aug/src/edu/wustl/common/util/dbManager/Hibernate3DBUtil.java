...
public class Hibernate3DBUtil
{
	...
	static
	{
		try
		{
			...
			InputStream inputStream = Hibernate3DBUtil.class.getClassLoader().getResourceAsStream(
					"dbutil.properties");
			...
			Hibernate3MetaData.initHibernateMetaData(cfg);

			Variables.databaseName = Hibernate3MetaData.getDataBaseName();
		}
		catch (Exception ex)
		{
			ex.printStackTrace();
	...
	private static void addConfigurationFile(String fileName, Configuration cfg, EntityResolver entityResolver)
	{
		...
		try
		{
			InputStream inputStream = Hibernate3DBUtil.class.getClassLoader().getResourceAsStream(fileName);
