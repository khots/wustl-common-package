...
public class OutputTreeNode implements IOutputTreeNode
{
	 ...
	 * @throws DuplicateChildException if there already exists a child with the given output Entity and association
			...
			throws DuplicateChildException
	{
		...
		//child does not exist with same outputEntity & association, so adding new child for the output tree node.
	...
	/**
	 * @return true if this node is leaf node. The node is leaf node if, its children list is empty.
	...
	/**
	 * @return true if this node is root node. The node is root node if its parent node is null.
	 ...
	 * @see edu.wustl.common.querysuite.queryobject.IOutputTreeNode#removeChild(edu.wustl.common.querysuite.queryobject.IAssociation, edu.wustl.common.querysuite.queryobject.IOutputEntity)
	 */
	...
	public boolean equals(Object obj)
	{
		...
		if (obj != null && this.getClass() == obj.getClass())
		{
			...
			if (this.outputEntity != null && this.outputEntity.equals(node.outputEntity)
					&& this.parentAssociation == null
					? node.parentAssociation == null
					: this.parentAssociation.equals(node.parentAssociation)
							&& this.parentNode == null ? node.parentNode == null : this.parentNode
	...
	public int hashCode()
	{
		...
		{
			hash = hash * Constants.HASH_PRIME + outputEntity.hashCode();
		}
		if (parentNode != null)
		{
			hash = hash * Constants.HASH_PRIME + parentNode.hashCode();
		}
		if (parentAssociation != null)
		{
			hash = hash * Constants.HASH_PRIME + parentAssociation.hashCode();
		}
		hash = hash * Constants.HASH_PRIME + children.hashCode();
		hash = hash * Constants.HASH_PRIME + childrenAssoiations.hashCode();
		return hash;
	}
	...
	{
		return outputEntity.toString() + parentNode;
	}
