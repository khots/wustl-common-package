...
public class NLevelCustomCombo extends TagSupport
{
	...
	private boolean validate()
	{
		...
		else if (initialValues == null || initialValues.length > numberOfCombosNeeded
	...
	private void initOptionalAttributes()
	{
		...
		else
		{
			...
			{
				if (initialValues[i] == null || initialValues[i].equals("")
	...
	private void getCombos(Object dMap)
	{
		...
		if ((dMap instanceof Map && ((Map) dMap).size() > 0) || (dMap instanceof List)
				&& (((List) dMap).size() > 0))
		{
			if (dMap instanceof Map)
			{
				Set keySet = ((Map) dMap).keySet();
				combosHTMLStr = combosHTMLStr + verticalCombosStart + "<td class=\""
						+ formLabelStyle + "\">" + "</td><td class=\"" + formLabelStyle
						+ "\" nowrap> " + "<select size=\"1\" name =\""
						+ attributeNames[comboCounter] + "\" style =\"" + styleClass
						+ "\" onmouseover =\"showTip(this.id)\""
						+ "\" onmouseout =\"hideTip(this.id)\"" + "\" class=\"" + tdStyleClass
				...
				if ((comboCounter == 0)
						|| (comboCounter > 0 && !(((String) initialValues[comboCounter - 1])
								.equals("-1"))))
				{
			...
			else if (dMap instanceof List) // Termination condition for recursion
			{
				...
				combosHTMLStr = combosHTMLStr + verticalCombosStart + "<td class=\""
						...
						+ "\" nowrap> " + "<select size=\"1\" name =\""
						+ attributeNames[comboCounter] + "\" style =\"" + styleClass
						+ "\" class=\"" + tdStyleClass + "\" id =\"customListBox_" + rowNumber
						+ "_" + comboCounter + "\"";
				if (disabled)
				{
					combosHTMLStr += " disabled = \"true\"";
				...
				if (comboCounter > 0 && !(((String) initialValues[comboCounter - 1]).equals("-1")))
				{
					for (int i = 0; i < dList.size(); i++)
					{
						...
						if (initialValues.length == numberOfCombosNeeded
								&& initialValues[comboCounter] != null
		...
		// to handle initial value condition were dMap.size() == 0 , NO RECURSSIVE CALL FROM HERE
		{
			...
			for (int i = 0; i < Integer.parseInt(noOfEmptyCombos); i++)
			{
				...
				combosHTMLStr = combosHTMLStr + verticalCombosStart + "<td class=\""
						+ formLabelStyle + "\" nowrap> " + "<select size=\"1\" name =\""
						+ attributeNames[comboCounter] + "\" style =\"" + styleClass
						+ "\" class=\"" + tdStyleClass + "\" id =\"customListBox_" + rowNumber
						+ "_" + comboCounter + "\"";
				if (disabled)
