		...
		try
		{
			...
			if (pageOf.equals(Constants.PAGEOF_STORAGE_LOCATION))
			{
				...
				treeType = Constants.STORAGE_CONTAINER_TREE_ID;
					...
					|| pageOf.equals(Constants.PAGEOF_MULTIPLE_SPECIMEN)
			...
			if (applicationPath.indexOf('/', 1) != -1)
			{ //indexOf returns -1 if no match found
				String newApplicationPath = null;
				newApplicationPath = applicationPath.substring(0,
						applicationPath.indexOf('/', 1) + 1);
				applicationPath = newApplicationPath;
			}

			//Kapil: MAC ISSUE JDK 1.3.1
			String urlSuffix = applicationPath + Constants.TREE_DATA_ACTION + ";jsessionid="
					+ session_id + "?" + Constants.PAGEOF + "=" + URLEncoder.encode(pageOf);
			if (pageOf.equals(Constants.PAGEOF_TISSUE_SITE) == true)
			{
				//Kapil: MAC ISSUE JDK 1.3.1
				urlSuffix = urlSuffix + "&" + Constants.PROPERTY_NAME + "="
						+ URLEncoder.encode(propertyName) + "&" + Constants.CDE_NAME + "="
						+ URLEncoder.encode(cdeName);
			}
			...
			{
				urlSuffix = urlSuffix + "&" + Constants.PROPERTY_NAME + "="
						+ URLEncoder.encode(propertyName) + "&" + Constants.SPECIMEN_TYPE + "="
						+ URLEncoder.encode(specimenType);
				urlSuffix = urlSuffix + "&" + Constants.SPECIMEN_CLASS + "="
			...
			if (pageOf.equals(Constants.PAGEOF_QUERY_RESULTS))
			{
				...
				spreadsheetViewRadioButton.setActionCommand(Constants.SPREADSHEET_VIEW);
				spreadsheetViewRadioButton.setSelected(true);
				spreadsheetViewRadioButton.setPreferredSize(new Dimension(80, 40));

				JRadioButton individualViewRadioButton = new JRadioButton(Constants.OBJECT_VIEW);
				...
				QueryResultsTreeListener nodeSelectionListener = new QueryResultsTreeListener(this
						.getCodeBase(), this.getAppletContext());
			...
			else if (pageOf.equals(Constants.PAGEOF_SPECIMEN_TREE))
			{
				...
				tree.setCellRenderer(new SpecimenTreeRenderer(specimenType, specimenClass));
					...
					|| pageOf.equals(Constants.PAGEOF_ALIQUOT))
			{
				StorageLocationViewListener viewListener = new StorageLocationViewListener(this
						.getCodeBase(), this.getAppletContext());
			...
			if (false == selectedNode.equals(new Long(0)))
			{
				urlSuffix = applicationPath + Constants.SHOW_STORAGE_CONTAINER_GRID_VIEW_ACTION
						...
						+ selectedNode.toString() + "&" + Constants.STORAGE_CONTAINER_TYPE + "="
						+ storageContainerType + "&" + Constants.STORAGE_CONTAINER_POSITION + "="
						+ position + "&" + Constants.PAGEOF + "=" + pageOf;
				dataURL = new URL(protocol, host, port, urlSuffix);
