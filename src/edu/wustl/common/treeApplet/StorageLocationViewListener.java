			...
			if (node != null)
			{
				...
				try
				{
					if ((!Constants.CATISSUE_CORE.equals(treeNode.getValue()))
							...
							&& (treeNode.getIdentifier() != null && treeNode.getIdentifier()
									.longValue() != 0) && !isSiteSelected(tree))
					{
						...
						{ //indexOf returns -1 if no match found
							...
							newApplicationPath = applicationPath.substring(0, applicationPath
									.indexOf('/', 1) + 1);
							applicationPath = newApplicationPath;
						}

						String urlSuffix = applicationPath
								+ Constants.SHOW_STORAGE_CONTAINER_GRID_VIEW_ACTION + "?"
								+ Constants.SYSTEM_IDENTIFIER + "=" + treeNode.getIdentifier()
								+ "&" + Constants.STORAGE_CONTAINER_TYPE + "="
								+ this.getStorageContainerType() + "&" + Constants.PAGEOF + "="
								+ this.pageOf;

						if (pageOf.equals(Constants.PAGEOF_SPECIMEN))
						{
							urlSuffix = applicationPath
									+ Constants.SHOW_STORAGE_CONTAINER_GRID_VIEW_ACTION + "?"
									+ Constants.SYSTEM_IDENTIFIER + "=" + treeNode.getIdentifier()
						...
						urlSuffix = urlSuffix + "&" + Constants.ACTIVITY_STATUS + "="
								+ treeNode.getActivityStatus();
