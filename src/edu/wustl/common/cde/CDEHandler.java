...
public class CDEHandler
{
		...
		if(obj!= null)
		{
			...
			else /** Retrieve CDE from caDSR */
			{
				try
				{
					...
					cde = cdeDownloader.downloadCDE(xmlCDE);
					//return cde;
				}
	...
	/**
	 *
	 * @param publicID String value
	 * @param cde CDE object
	 * @return CDE object
	 */
	private CDE loadCDEFromDB(String publicID, CDE cde)
	{
		...
		try
		{
			...
			{
				cde1 =  (CDE)list.get(0);
				inMemCacheMap.put(publicID, cde1);
			}
		}
		catch(BizLogicException ex)
		{
			Logger.out.debug(ex.getMessage(), ex);
			//throw new Exception("Could not load CDE "+CDEName+"
			//from local DB database: "+ ex.getMessage(), ex);
		}
		return cde1;
	}
}