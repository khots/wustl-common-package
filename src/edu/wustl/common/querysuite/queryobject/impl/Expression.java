 ...
 */
public class Expression extends BaseQueryObject implements IExpression {
     ...
     */
    public Expression(
            IQueryEntity constraintEntity,
            List<IExpressionOperand> expressionOperands,
            List<ILogicalConnector> logicalConnectors,
     ...
     * 
     * @hibernate.list name="expressionOperands" cascade="all-delete-orphan" inverse="false" lazy="false"
     ...
     * 
     * @hibernate.list name="logicalConnectors" cascade="all-delete-orphan" inverse="false" lazy="false"
     * @hibernate.collection-key column="QUERY_EXPRESSION_ID"
     * @hibernate.collection-index column="POSITION" TYPE="integer"
     * @hibernate.collection-one-to-many class="edu.wustl.common.querysuite.queryobject.impl.LogicalConnector"
     * @hibernate.cache usage="read-write"
     */
    public List<ILogicalConnector> getLogicalConnectors() {
     ...
     */
    public void setLogicalConnectors(List<ILogicalConnector> logicalConnectors) {
        this.logicalConnectors = logicalConnectors;
    }
    ...
    public int addOperand(IExpressionOperand operand) {
        ...
        if (expressionOperands.size() != 1) {
            logicalConnectors.add(QueryObjectFactory.createLogicalConnector(LogicalOperator.Unknown));
        }
     ...
     * @param isVisible true if this Expression should be visible; false otherwise
     */
     ...
     */
    public void setLogicalConnector(int leftOperandIndex, int rightOperandIndex, ILogicalConnector logicalConnector) {
        if (rightOperandIndex == leftOperandIndex + 1) {
    ...
    public int indexOfConnectorForOperand(IExpressionOperand operand) {
        ...
        if (index != -1) {
            ...
            {
                int preNesting = ((LogicalConnector) logicalConnectors.get(index - 1)).getNestingNumber();
                int postNesting = ((LogicalConnector) logicalConnectors.get(index)).getNestingNumber();
    ...
    public boolean isPseudoAnded(IExpressionId expressionId, IConstraints constraints) {
        ...
        } else if (immediateOperandIndex == Expression.BOTH_LOGICAL_CONNECTOR) // both logical connector sorrounding this expression have same nesting no. need to check 'And' operator for both.
        {
            ...
            if (operand.isSubExpressionOperand()
                    && LogicalOperator.And.equals(getLogicalConnector(preIndex, index).getLogicalOperator())) {
            ...
            if (operand.isSubExpressionOperand()
                    && LogicalOperator.And.equals(getLogicalConnector(index, postindex).getLogicalOperator())) {
                IExpression expression = constraints.getExpression((IExpressionId) operand);
                return isHavingSameClass(currentExpression, expression);
            }
        } else
        ...
        {
            if (LogicalOperator.And.equals(logicalConnectors.get(immediateOperandIndex).getLogicalOperator())) {
