...
public class PDFXMLParser {
	...
	throws IOException, DOMException, ParserConfigurationException, SAXException {
		...
		root = (Element)dom.getElementsByTagName("rows").item(0);
	...
	private void extractOuterData(){
		...
		labelFont = new String[nodeList.getLength()];
		for(int i = 0 ;i < nodeList.getLength(); i++){
			...
			Element label = (Element) element.getElementsByTagName("label").item(0);
			String font = label.getAttribute("font");
			if(font!=null){
				labelFont[i] = font;
			}else{
				labelFont[i] = "";
