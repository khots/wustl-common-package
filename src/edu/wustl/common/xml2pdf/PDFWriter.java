...
public class PDFWriter {
	...
	private void writeOuterData() throws Exception{
		String[] label = parser.getLabel();
		String[] text = parser.getText();
		String[] labelFont = parser.getLabelFont();
		double x = offsetLeft ;
		double y = offsetTop ;
		Font boldObliqueFont = new Font(pdf, "Helvetica-BoldOblique");
		boldObliqueFont.setSize(10);
		TextLine textLine ;
		if(label != null && text != null)
		for(int i = 0;i < label.length; i++){
			if(labelFont[i].equals("boldOblique")){
				y +=   boldObliqueFont.getSize();
				textLine = new TextLine(boldObliqueFont, label[i] + " " + text[i]);
			}else{
				y +=   f1.getSize();
				textLine = new TextLine(f1, label[i] + " " + text[i]);
			}
			textLine.setColor(RGBColor.getColor(pageTextColor));
			textLine.setPosition(x, y);
			textLine.drawOn(page);
		}
		offsetTop = y + f1.getSize();
	}
