...
public class UserBizLogic extends DefaultBizLogic
{
			...
			throws DAOException, UserNotAuthorizedException
	{
		...
		try
		{
			...
			if (Constants.PAGEOF_SIGNUP.equals(user.getPageOf()))
			{
				...
				privilegeManager.insertAuthorizationData(null, protectionObjects, null, user
						.getObjectId());
	...
	private User getUserObject(User user, DAO dao) throws DAOException, UserNotAuthorizedException
	{
		...
		object = dao.retrieve(CancerResearchGroup.class.getName(), userObj.getCancerResearchGroup()
				.getId());
	 ...
	 * @throws DAOException
	 */
	private void deleteCSMUser(gov.nih.nci.security.authorization.domainobjects.User csmUser)
			throws DAOException
	{
		try
		{
			...
			{
				SecurityManager.getInstance(ApproveUserBizLogic.class).removeUser(
		...
		try
		{
			...
			if (!userList.isEmpty())
			{
				...
				if (appUser.getCsmUserId() != null)
				{
					...
					Role role = SecurityManager.getInstance(UserBizLogic.class).getUserRole(
